...
setopt PROMPT_SUBST
export PROMPT='${NEWLINE}$PROMPT_TIME %F{red}%F{white} $(ruby_version)%F{grey} %F{white}$(git_branch)%F{grey}${NEWLINE}%F{blue}%~%F{grey} ${NEWLINE}%F{blue}~%F{grey} ${PS1_EMOJIS[$RANDOM % $NUMBER_OF_EMOJIS]} '
